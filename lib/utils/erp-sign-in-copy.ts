...
export function getSignInCopy(erpBase: string) {
  ...
  if (livro) {
    return {
      ...
      footer: "No separate registration — first login creates your BBAI profile, then onboarding.",
      siteLabel: "Livro",
      host,
  ...
  return {
    ...
    submitLabel: "Sign in",
    footer: `Signing in against ${host} — first login creates your BBAI profile, then onboarding.`,
    siteLabel: host,
    host,
